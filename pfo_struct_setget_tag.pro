...
;   /set or /get (one required): set or get tag

;   taglist_series: a [list of] top-level tags in the parinfo
;       structure which will be procesed one-by-one, in series.  The
;       keywords passed with the _REF_EXTRA mechanism are passed on to
;       the __set/get_tag procedures of the tags and "soaked up" as
;       they are encountered.  In other words, the first __set/get_tag
...
;       unintended results.  It is therefore safest to specify
;       specific top-level tags

;   strict: use the a _STRICT_EXTRA-like construct to make
;   	sure _all_ of the tags in a taglist_series are used.

;   parallel_taglist: like taglist_series, but all _REF_EXTRA keywords
;          are always passed to all of the __set/get_tag routines in
;          the parallel_taglist list
...
;
; $Id: pfo_struct_setget_tag.pro,v 1.2 2011/09/01 22:14:50 jpmorgen Exp $
;
; $Log: pfo_struct_setget_tag.pro,v $
; Revision 1.2  2011/09/01 22:14:50  jpmorgen
...
pro pfo_struct_setget_tag, parinfo, idx=idx, $
  ...
  parallel_taglist=parallel_taglist, $
  strict=strict, $
  next=next, $
  _REF_EXTRA=extra
  ...
  N_tlp = N_elements(parallel_taglist)
  if N_tls gt 0 and N_tlp gt 0 then $
      message, 'ERROR: specify either taglist_series or parallel_taglist'
  ...
  ;; parallel_taglist case.  Just call each <tag>_struct__set/get_tag
  ;; routine with all of the _EXTRA parameters.
  if N_tlp gt 0 then begin
     ;; Check for the /parallel_taglist or parallel_taglist=0 cases
     if size(/type, parallel_taglist) ne !tok.string and N_tlp eq 1 then begin
        ;; If parallel_taglist=0, we are trying to turn off taglist
        ;; processing
        if parallel_taglist eq 0 then $
          return
        ;; Here is our /parallel_taglist case.
        parallel_taglist = tag_names(parinfo) 
     endif ;; checking for /parallel_taglist
     ;; Sanity check taglist
     if size(/type, parallel_taglist) ne !tok.string then $
       message, 'ERROR: parallel_taglist must be a string'
     for it=0, N_tlp-1 do begin
        ...
        if parallel_taglist[it] eq 'TOP_LEVEL_STRUCT' then $
          parallel_taglist[it] = parinfo[0].top_level_struct
        ;; Justify the names of our tags and top-level structure.  The
        ;; top_level_struct is the full structure name (including
        ;; _struct), the tags don't include _struct.  Assume we have a
        ;; tag...
        setget_tag_proc = parallel_taglist[it]+'_struct__'+setget+'_tag'
        ;; ...and check for the top-level structure
        spos = strpos(parallel_taglist[it], '_STRUCT', /reverse_search)
        if spos eq strlen(parallel_taglist[it]) - strlen('_STRUCT') then $
          setget_tag_proc = parallel_taglist[it]+'__'+setget+'_tag'
        ...
        call_procedure, setget_tag_proc, parinfo, idx=idx, _EXTRA=extra
     endfor ;; each tag in parallel_taglist
  endif ;; parallel_taglist
