; $Id: pfo_cw_field.pro,v 1.1 2011/09/01 22:18:57 jpmorgen Exp $
; $log:$
;
...
;   The field consists of a label and a text widget.  PFO_CW_FIELD's can
...
;
; Wed Aug 17 21:21:56 2011  jpmorgen@sandusky

; kbrd_focus_events: like return_events, but returns an event when
; keyboard focus is lost (e.g. user tabs away from the widget)
...
;
;       base = WIDGET_BASE()
;       field = PFO_CW_FIELD(base, TITLE="Name:", /FRAME)
...
;
FUNCTION PFO_CW_FIELD_VALIDATE, Value
...
FUNCTION PFO_CW_FIELD_VALUE, Value, Type

    COMPILE_OPT hidden

    IF Type EQ 0 THEN RETURN, Value

    NValue  = PFO_CW_FIELD_VALIDATE(Value[0])
...
;
;   Procedure to set the value of a PFO_CW_FIELD
;
PRO PFO_CW_FIELD_SET, Base, Value
...
;;        SET_VALUE=STRTRIM(PFO_CW_FIELD_VALUE(sValue, State.Type),2)
    WIDGET_CONTROL, State.TextId, $
        SET_VALUE=strtrim(string(format=state.format, PFO_CW_FIELD_VALUE(sValue, State.Type)), 2)
    WIDGET_CONTROL, Child, SET_UVALUE=State, /NO_COPY
END

;
;   Function to get the value of a PFO_CW_FIELD
;
FUNCTION PFO_CW_FIELD_GET, Base
    ...
    Ret = PFO_CW_FIELD_VALUE(Value, State.Type)
...
;
PRO PFO_CW_FIELD_INT, Ch, State, Event, Altered
...
FUNCTION PFO_CW_FIELD_EXPONENT, Value, Idx
...
;
PRO PFO_CW_FIELD_FLOAT, Ch, State, Event, Altered
    ...
    IF Ch EQ 10b THEN BEGIN
    Value   = PFO_CW_FIELD_VALIDATE(Value)
    ...
    46b:    BEGIN
        ...
        IF PFO_CW_FIELD_EXPONENT( Value, Idx ) THEN BEGIN
    ...
    END
    45b:    BEGIN
        HaveExp = PFO_CW_FIELD_EXPONENT( Value, Idx )
...
FUNCTION PFO_CW_FIELD_EVENT, Event
  ...
  ;; Wed Aug 17 20:19:05 2011  jpmorgen@sandusky
  ;; trying to make loss of keyboard focus do the same thing as cr
  sn = tag_names(event, /structure_name)
  if sn eq 'WIDGET_KBRD_FOCUS' then begin
     ;; Don't do anything when we arrive in the widget
     if event.enter eq !tok.yes then begin
        WIDGET_CONTROL, StateHolder, SET_UVALUE=State, /NO_COPY
        return, 0L
     endif ;; swallow focus enter events
     
     ;; If we made it here, we have a leave event.
     altered = 2^2
  endif else begin

     ;; Non keyboard focus (original code)
     ...
     IF State.Type NE 0 THEN BEGIN ; Not a String?
        ...
        IF Event.Type LE 1 THEN BEGIN ; Insert Characters?
           ...
           IF State.Type EQ 1 THEN Procedure='PFO_CW_FIELD_FLOAT' $
           ELSE Procedure='PFO_CW_FIELD_INT'
        ...
        ENDIF
     ENDIF ELSE BEGIN
        ;;   All delete/add char events effect the contents of a
        ;;   string. <CR> is considered special.
  ...
  ;; Wed Aug 17 21:46:46 2011  jpmorgen@sandusky
  ;; Altered = 0 if nothing was changed, 2^0 if something changed,
  ;; 2^1, if <CR> was pressed and 2^2 if the widget lost keyboard focus
  ...
  IF State.Update NE 0 AND $
     (Altered AND State.Update) gt 0 THEN BEGIN

     WIDGET_CONTROL, State.TextId, GET_VALUE=Value
     RValue  = PFO_CW_FIELD_VALUE(Value, State.Type)
...
FUNCTION PFO_CW_FIELD, $
   Parent, COLUMN=Column, ROW=Row, $
   ...
   kbrd_focus_events=kbrd_focus_events, format=format
;   FLOOR=vmin, CEILING=vmax
    ...
    ;; Wed Aug 17 21:39:50 2011  jpmorgen@sandusky
    ;; Make a bitmask for the update flag
    Update      = 0
    IF KEYWORD_SET(AllUpdates) THEN Update  += 2^0
    IF KEYWORD_SET(ReturnEvents) THEN Update   +=  2^1
    IF KEYWORD_SET(kbrd_focus_events) THEN Update    += 2^2
    ...
    IF N_ELEMENTS(uname) EQ 0 THEN uname='PFO_CW_FIELD_UNAME'
    ...
    Base    = WIDGET_BASE(Parent, ROW=Row, COLUMN=Column, UVALUE=UValue, $
            EVENT_FUNC='PFO_CW_FIELD_EVENT', $
            PRO_SET_VALUE='PFO_CW_FIELD_SET', $
            FUNC_GET_VALUE='PFO_CW_FIELD_GET', $
            FRAME=Frame, UNAME=uname )

    if ( N_ELEMENTS(tab_mode) ne 0 ) then $
        WIDGET_CONTROL, Base, TAB_MODE = tab_mode

    ;; Set default format for floating point
    ...
    Label   = WIDGET_LABEL(Base, VALUE=Title, FONT=LabelFont, $
            UNAME=uname+'_LABEL')
    Text    = WIDGET_TEXT(Base, VALUE=strtrim(string(format=format, TextValue), 2), $
