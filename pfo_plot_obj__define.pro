...
;
; $Id: pfo_plot_obj__define.pro,v 1.1 2011/08/01 19:18:16 jpmorgen Exp $
;
...
pro pfo_plot_obj::plot, $
   ...
   xlog=xlog, $	;; main window and deviates reads in log X
   ylog=ylog, $ ;; plot of main window reads in log Y.  --> Deviates always linear
  ...
  if N_elements(Xin_title) eq 0 then Xin_title = self.Xin_title
  if N_elements(Xaxis_title) eq 0 then Xaxis_title = self.Xaxis_title
  if N_elements(Yin_Xin_title) eq 0 then Yin_Xin_title = self.Yin_Xin_title
  if N_elements(Yin_Xaxis_title) eq 0 then Yin_Xaxis_title = self.Yin_Xaxis_title
...
function pfo_plot_obj::descr
  ...
  descr = *self.ppfo_plot_obj_descr
  return, descr
end
...
function pfo_plot_obj::init, $
  ...
  self.ppfo_plot_obj_descr $
     = ptr_new( $
     {README	: 'pfo_plot_obj encapsulates all of the information necessary for plotting PFO functions.', $
  ...
  self.plot_window_charsize = 1
  self.plot_TT_font_name = 'Times*24'

  self.Xin_title       = !pfo.Xin_title      
  self.Xaxis_title     = !pfo.Xaxis_title    
  self.Yin_Xin_title   = !pfo.Yin_Xin_title  
  self.Yin_Xaxis_title = !pfo.Yin_Xaxis_title
...
pro pfo_plot_obj__define
  ...
  objectClass = $
     {pfo_plot_obj, $
      ...
      plot_window_charsize: 0., $ ;; charsize in regular graphics windows
      ...
      Xin_title		: '', $ ;; title for X-axis plots when in Xin mode (e.g. channels)
      Xaxis_title	: '', $ ;; title for X-axis plots when in Xaxis mode (e.g. keV)
      Yin_Xin_title	: '', $ ;; title for Y-axis plots when Yunits=!pfo.Xin regardless of Xunits value (e.g. counts/channel)
      Yin_Xaxis_title	: '', $ ;; title for X-axis plots when Xunits=!pfo.Xaxis and Yunits=!pfo.Xaxis (e.g. counts/keV)
