; +
; $Id: pfo_poly.pro,v 1.1 2003/12/19 00:02:05 jpmorgen Exp jpmorgen $
...
;; coefficients with 1.0010, 1.0011, etc.  Note that the polynomial
...
;; round things to get the digets right, the 9th polynomial can't be
...
function pfo_poly, Xin, params, dparams, parinfo=parinfo, idx=idx, $
  ...
  ;; things into integers.  p[br]nums will be between 1 and 9 when
  ;; they are valid for that particular parameter type.  
  ftypes = parinfo[f_idx].pfo.ftype - fn
  pbnums = round(ftypes * 10.  ) ;; Boundaries
  pbidx = where(0 lt pbnums and pbnums lt 10, npb)
  ;; unwrap
  if npb gt 0 then $
    pbidx = f_idx[pbidx]
  prnums = round(ftypes * 100. ) ;; Reference values
